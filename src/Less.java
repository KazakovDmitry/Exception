...
public class Less {
    ...
    static int processArray(int[] arr, int searchNumber){
        if (arr == null){
            return -3; // Некорректно проинициализирован
        }
        if (arr.length < 3){
            return -1; //Длина массива меньше 3
        }
        int searchRes = -2;
        ...
        for (int i = 0; i < arr.length; i++) {
            for (int j = 0; j < arr[i].length; j++) {
                if (arr[i][j] != 0 && arr[i][j] != 1){
                    throw new RuntimeException(String.format("Некорректное значение масcива по индексу [%d][%d]", i, j));
                }
                else{
                    sum += arr[i][j];
                }
